...
int main() {
    std::thread t_sum(thread_sum, 5, 6); // В С++ возможно передавать еще аргументы функции. Для этого 
                                         // просто через запятую перечисляем аргументы, которые хотим передавать 
                                        // Можно передавать любое количество аргументов, но оно должно
    ...
    // std::thread t_append(thread_append, std::ref(a)); // Когда хотим передавать по ссылке, используем std::ref. ref отличается
