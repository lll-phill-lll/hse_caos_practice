...
// пробуем симитировать запуск программы из shell ./program < file_in > file_out
// 2>> file_err она берет два числа, одно из них печатает в stdout, второе в
// stderr
...
int main() {
    ...
    if (!pid) {
        ...
        execl("./program", "./program", NULL);  // запускаем нашу программу
        _exit(1);
    }
    wait(NULL);
}
