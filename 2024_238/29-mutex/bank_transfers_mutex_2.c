...
void transfer(Account* from_account, Account* to_account, int amount) {
    ...
    Account* first_account = (from_account < to_account) ? from_account : to_account;
    Account* second_account = (from_account < to_account) ? to_account : from_account;

    // блокируем два mutex-а, откуда хотим снять деньги и куда хотим зачислить
    // 1 -> 2 (lock(1)) -> lock(1)
    // 2 -> 1 (lock(2)) -> lock(1)
    // 2 -> 2
    pthread_mutex_lock(&first_account->m);
    pthread_mutex_lock(&second_account->m);
    ...
    pthread_mutex_unlock(&second_account->m);
    pthread_mutex_unlock(&first_account->m);
}
...
int main() {
    ...
    pthread_mutex_init(&mall_account.m, &attr);
